...
export async function run(argv) {
  ...
  console.log(`Tessera up — scope=${scope}`)
  ...
  else console.warn('  ⚠ tmux not found — spawning detached (setsid). Use `tessera ps` to monitor.')
  const launched = []
  for (let i = 1; i <= n; i++) {
    ...
    if (mode === 'isolated') {
      ...
      try { execFileSync('git', ['-C', scope, 'worktree', 'add', '-b', `tessera/${label}`, wt], { stdio: 'ignore' }); cwd = wt }
    ...
    const windowCmd = `cd ${shq(cwd)} && export TESSERA_LABEL=${shq(label)} TESSERA_TASK=${shq(task)} TESSERA_TOUCHES=${shq(touches)} TESSERA_ROLE=agent && ${agentCmd}`
    if (tmux) {
  ...
  console.log(`✓ launched ${launched.length} agent(s). Monitor: tessera ps --follow   Teardown: tessera kill <label>`)
}
