...
import { DEFAULT_MARKERS, tesseraDir } from './scope.mjs'

export const DEFAULTS = {
  dir: '.tessera',
  bus_path: null,                                  // default <scope>/.tessera/bus.ndjson; set to reuse an existing dir
  markers: DEFAULT_MARKERS,
  boundary: process.env.TESSERA_ROOT || null,
...
export function loadConfig(scope) {
  const cfg = { ...DEFAULTS }
  const f = join(tesseraDir(scope, cfg), 'config.json')
  if (existsSync(f)) {
