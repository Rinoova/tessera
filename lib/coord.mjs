...
import { agentsyncDir, busPath, enc, realpathM, enclosingGitRoot } from './scope.mjs'
import { Bus } from './bus.mjs'

export const HEARTBEAT_TTL_MS = 120_000

export function statePaths(scope, cfg = {}) {
  const base = agentsyncDir(scope, cfg)
  return { base, state: join(base, 'state'), agents: join(base, 'state', 'agents'), bus: busPath(scope, cfg) }
}

// A scope participates iff it has been opted in (.agentsync/ exists), or the
// launcher set AGENTSYNC_TOUCHES, or the user globally enabled AGENTSYNC_AUTO=1.
// Otherwise the hook is a near-instant no-op (cheap in non-participating projects).
export function participates(scope, cfg = {}) {
  if (process.env.AGENTSYNC_AUTO === '1') return true
  if ((process.env.AGENTSYNC_TOUCHES || '').split(',').filter(Boolean).some(t => realpathM(t) === scope)) return true
  return existsSync(agentsyncDir(scope, cfg))
}
...
function relIgnore(gitRoot, scope, cfg) {
  const rel = relative(gitRoot, agentsyncDir(scope, cfg)) || '.agentsync'
  return rel + '/'
}
...
export function peerDigest(scope, cfg, selfId) {
  ...
  return `AgentSync: ${peers.length} other agent(s) active in this scope (${scope}):\n${lines.join('\n')}\nCoordinate: avoid concurrently rewriting the same files; use \`agentsync ps\` for live status.`
}
...
export function overlapWarning(scope, cfg, selfId, target) {
  ...
  return `AgentSync: ${clash.map(p => p.label || p.id.slice(0, 8)).join(', ')} is also touching ${rel}. Coordinate before overwriting.`
}
