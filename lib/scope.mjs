...
// The marker *order* is only a same-dir tiebreak. `.tessera-scope` is the sole
...
export function scopeRoot(target, opts = {}) {
  ...
  let cur = startDir
  while (true) {
    if (override == null && existsSync(join(cur, '.tessera-scope'))) override = cur
...
export function tesseraDir(scope, cfg = {}) { return join(scope, cfg.dir || '.tessera') }
export function busPath(scope, cfg = {}) {
  return cfg.bus_path ? join(scope, cfg.bus_path) : join(tesseraDir(scope, cfg), 'bus.ndjson')
}
