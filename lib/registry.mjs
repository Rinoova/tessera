...
import { appendFileSync, readFileSync, writeFileSync, renameSync, chmodSync, mkdirSync } from 'node:fs'
...
export function cacheDir() {
  ...
  try { chmodSync(d, 0o700) } catch {}   // tighten a pre-existing dir created with a looser umask
  return d
}
...
export function sessionPeers(scope, selfSid) {
  ...
  if (live.length !== recs.length) {                       // GC only when it changes something
    // Atomic rewrite (tmp + rename) so a concurrent reader never sees a half-written file.
    try {
      const tmp = sessFile() + '.tmp.' + process.pid
      writeFileSync(tmp, live.map(r => JSON.stringify(r)).join('\n') + (live.length ? '\n' : ''), { mode: 0o600 })
